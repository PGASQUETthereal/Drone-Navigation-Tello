...
def getKeyboardInput():
    ...
    if kp.getKey("z"): ud = liftSpeed #Controlling The Up And Down Movemnt:
    elif kp.getKey("s"): ud = -liftSpeed 

    if kp.getKey("d"): yv = rotationSpeed #Controlling the Rotation:
    elif kp.getKey("q"): yv = -rotationSpeed 

    if kp.getKey("a"): Drone.land(); time.sleep(3) #Landing The Drone
